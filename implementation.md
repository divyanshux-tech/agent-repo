...
```
User → Orchestrator Agent → Destination Agent → [Travel Agent | Hotel Agent | Activity Agent] (parallel)
     → Trip Expense Estimator → Budget Optimizer → Plans shown to user
...
| Travel Agent | Curate flight/travel candidates | Spend the whole budget on travel |
| Hotel Agent | Curate hotel candidates | Decide final budget allocation |
| Activity Agent | Curate activities/experiences | Force activities as mandatory |
...
## Phase 4 — Travel, Hotel & Activity Agents (Parallel Specialists)

**Goal:** Once destination is locked, run the three specialist agents in parallel to build curated candidate sets.

### 4.1 Travel Agent
**Requirements:**
- Integration with at least one flight/travel provider API (MVP: one provider is enough).
- Filtering logic: correct route/dates/passenger count, remove unavailable options, apply user preferences (e.g., non-stop only).
- Return top N curated candidates, each retaining the full original payload plus a `source_reference`.
...
```
**Critical rule:** `T1.price = ₹8,000` is the actual provider price, not an invented budget allocation.
...
- Return multiple candidates spanning different price/quality tiers.
...
```json
{
  ...
  "source_reference": "HOTEL_RESULT_xyz"
}
```

### 4.3 Activity Agent
**Requirements:**
- Fit destination, dates, interests, duration, price.
- MVP fallback: internal curated activity catalog if no live API is integrated yet.
- Activities are optional — user can opt out entirely.

**Output (example):**
```json
{
  "id": "A1",
  "activity": "North Goa sightseeing",
  "date": "2026-10-17",
  "duration_hours": 6,
  "price_total": 2000,
  "currency": "INR",
...
### 4.4 Cross-Cutting Requirements for All Three Agents
- Run in parallel (async/concurrent execution) to minimize latency.
- Persist candidates into `trip_candidates` with `type`, `provider_reference`, and `expires_at` (since prices go stale).
- Never hard-allocate a fixed sub-budget per category.

### 4.5 Acceptance Criteria
- [ ] All three agents can execute concurrently and return within an acceptable latency SLA (define, e.g., < 5s combined for MVP).
- [ ] Each candidate is traceable back to its raw provider response via `source_reference`.
- [ ] Hotel/Travel/Activity candidates persist correctly with expiry timestamps.
...
4. Integrate **one** travel provider/API + Travel Agent filtering.
5. Integrate **one** hotel provider/API + Hotel Agent filtering.
6. Activity Agent using a small internal catalog (no live API required yet).
7. Destination cost-profile table for food/local transport estimates.
8. Simple combination optimizer using enumeration.
9. Show 3–5 feasible plans to the user.
10. Add change/replan commands.
11. Implement booking/revalidation — only after selection.
12. **Later:** itinerary-based transport estimation, more providers, more destinations, stronger optimization (solver-based), analytics/ML pipeline.
